...
drawmenu(void)
{
	...
	if (prompt && *prompt) {
		drw_setscheme(drw, scheme[SchemeSel]);
		px = drw_text(drw, x, 10, promptw, bh, lrpad / 2, prompt, 0) - 20; // Dodgy hardcoded shift left for prompt
	}
	/* draw input field */
	w = (lines > 0 || !matches) ? mw - x : inputw;
	drw_setscheme(drw, scheme[SchemeSel]);
	drw_text(drw, px, 10, w, bh, lrpad / 2, text, 0);

	curpos = TEXTW(text) - TEXTW(&text[cursor]);
	if ((curpos += lrpad / 2 - 1) < w) {
		drw_setscheme(drw, scheme[SchemeNorm]);
		drw_rect(drw, px + curpos, 15, 2, bh - 10, 1, 0);
	}
